...
const network = process.env.BLOCKRADAR_NETWORK || 'testnet'; // 'testnet' or 'mainnet'
...
/**
 * Create a dedicated address for a user
 * BlockRadar uses Master Wallets (created via dashboard) and Dedicated Addresses (created via API)
 * @param userName - User's name for the address
 * @param userMetadata - Additional metadata to attach to the address
 */
export async function createDedicatedAddress(userName: string, userMetadata?: any) {
  ...
  const payload = {
    ...
    enableGaslessWithdraw: true, // Allow gasless withdrawals
  };
...
/**
 * Send crypto using BlockRadar Gateway
...
export async function sendCrypto(params: { 
  ...
  const payload = {
    ...
    blockchain: params.blockchain || 'base', // Default to Base network
...
/**
 * Get address balance (if needed in the future)
 * Note: BlockRadar webhooks will notify of deposits, so balance tracking should be done in your database
 */
